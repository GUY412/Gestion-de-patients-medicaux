    ...
    <div class="flex min-h-screen">
        ...
        <aside class="hidden w-64 shrink-0 flex-col border-r border-border/70 bg-card/40 lg:flex">
            ...
            <nav class="flex-1 space-y-1 p-4">
                <p class="px-3 pb-2 pt-2 text-xs font-medium uppercase tracking-wider text-muted-foreground/70">Menu</p>

                <a href="{{ route('dashboard') }}"
                   class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors
                   {{ request()->routeIs('dashboard') ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-secondary hover:text-foreground' }}">
                ...
                @if (auth()->user()->role === 'admin')
                    <p class="px-3 pb-2 pt-6 text-xs font-medium uppercase tracking-wider text-muted-foreground/70">Administration</p>
            ...
            <div class="border-t border-border/70 p-4">
                <div class="flex items-center gap-3 px-2 py-2">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="mt-0.5 text-xs text-muted-foreground capitalize">{{ auth()->user()->role }}</span>
                    </div>
                </div>
                    ...
                    @isset($header)
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                            <div>
                                ...
                                <h1 class="mt-1 font-display text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">{{ $header }}</h1>
                            </div>
                        </div>
                    @endisset
